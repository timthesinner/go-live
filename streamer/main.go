...
func (s *webmStream) ServeHTTP(res http.ResponseWriter, req *http.Request) {
	...
	if r := _head.FindStringSubmatch(req.URL.Path); len(r) != 0 {
		if head, err := strconv.ParseInt(r[1], 10, 64); err == nil {
			if offset == 0 {
...
//tools.exe | C:\ffmpeg\bin\ffmpeg.exe -re -i pipe: -c:v libvpx -c:a libvorbis -f webm -b:v 1250K -speed 10 -threads 10 -vf scale=-1:576 output.webm
func main() {
	flag.Parse()
